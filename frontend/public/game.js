...
const { REGIONS, getRegionOrder } = window.rcGame.state;
const { getCardDef, getCardIdByChampionName, getCardImagePath } =
  window.rcGame.cards;
const { actingPlayerIndexForTurnTimer, turnTimerKeyForState } =
  window.rcGame.timers;
const socket = window.rcGame.socket.createSocket(attachFirebaseAuthToSocket);
